...
forger_stop ()
{
    ...
    pm2 stop phantom-core-forger >> "$commander_log" 2>&1
...
forger_delete ()
{
    ...
    if [[ -n $delete_forger ]]; then
        ...
        pm2 delete phantom-core-forger >> "$commander_log" 2>&1
...
forger_status ()
{
    local status=$(pm2status "phantom-core-forger" | awk '{print $10}')
...
__forger_start_with_bip38 ()
{
    ...
    pm2 start "$CORE_DIR/packages/core/bin/phantom" --name phantom-core-forger -- forger --data "$CORE_DATA" --config "$CORE_CONFIG" --token "$CORE_TOKEN" --network "$CORE_NETWORK" --bip38 "$bip38" --password "$password" >> "$commander_log" 2>&1
}

__forger_start_without_bip38 ()
{
    pm2 start "$CORE_DIR/packages/core/bin/phantom" --name phantom-core-forger -- forger --data "$CORE_DATA" --config "$CORE_CONFIG" --token "$CORE_TOKEN" --network "$CORE_NETWORK" >> "$commander_log" 2>&1
}
